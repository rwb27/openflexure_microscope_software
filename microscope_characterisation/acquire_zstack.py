...
if __name__ == "__main__":
    try:
        ...
        z_shift = int(sys.argv[2])
        n_shifts = int(sys.argv[3])
    except IndexError:
        output_dir = "--help"

    if "--help" in output_dir:
        print "Usage: acquire_zstack.py <output_dir> <dz> <layers>"
        exit(0)
    ...
    with picamera.PiCamera(resolution=microscope.picam2_full_res) as camera, \
        ...
        scan_points = ii[:, np.newaxis] * np.array([0,0,z_shift])[np.newaxis,:]

        for i in ms.scan_linear(scan_points):
            time.sleep(1)
            camera.capture(os.path.join(output_dir,"zstack_%03d_x%d_y%d_z%d.jpg" % ((i,) + tuple(stage.position))), use_video_port=False)
        time.sleep(0.5)
